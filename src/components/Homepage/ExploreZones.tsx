...
const ExploreZones = () => {
  const [stars, setStars] = useState<{ id: number; size: number; duration: number; delay: number; left: number; top: number }[]>([]);
          ...
          {stars.map((star) => (
            <div
              ...
              style={{
                ...
                left: `${star.left}%`,
                top: `${star.top}%`,
                animation: `float ${star.duration}s ease-in-out ${star.delay}s infinite, twinkle ${
                  star.duration * 0.7
                }s ease-in-out ${star.delay}s infinite`,
              }}
            />
          ))}
        </div>
