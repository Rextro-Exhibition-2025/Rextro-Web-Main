...
CrackTheCircuit.GameState.prototype = {
    ...
    create: function() {
        ...
        this.privateMode = this.isPrivateMode();
...
//        });
    ...
    initLevelWin: function() {
        ...
        // Send level win update
        this.sendProgressUpdate('LEVEL_WIN');
    ...
    nextLevelPressed: function() {
        ...
        // Check if this was the last level before incrementing
        if(this.curLevelNum >= this.totalLevels) {
            this.sendProgressUpdate('GAME_COMPLETE');
        }
    ...
    sendProgressUpdate: function(eventType) {
        if (window.parent && window.parent !== window) {
            try {
                window.parent.postMessage({
                    type: eventType,
                    gameData: {
                        currentLevel: this.curLevelNum,
                        totalLevels: this.totalLevels,
                        levelsBeaten: this.numLevelsBeaten,
                        hintsUsed: this.hintsUsedCount,
                        elapsedTime: Date.now() - this.gameStartTime
                    }
                }, '*');
            } catch (e) {
                console.log('Failed to send progress update:', e);
            }
        }
    },
}